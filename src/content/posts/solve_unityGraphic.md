---
title: Vulkanは全てを解決するって行ったじゃんね
published: 2026-03-15
draft: true
description: WIP. 現状うまく行ってません。UnityでVulkanを使って正しい表示の元改変しようとした際のメモ。
tags:
  - linux
  - VRChat
  - Unity
---
## 結論
弊環境(RTX50X0+Ubuntu22.04)では現状vulkanで動いてないです
2022.3.22f1 → vulkanで動かない
2022.3.60f3 → vulkanでOOM
## 記事中でやっていること
- Resizable BARが必要
- CSMを無効にする必要がありEFIが必要
- vulkanは2022.3.22f1は非動作、新しいBuildは動く(60f3で改変→22f1でupload)
## 調べた/やったこと
1. そもそもUbuntuのMesaDriverは、**種類ごとに分けられていない**
	`mesa-vulkan-drivers`を取得すると、Intel/Nvidia/Radeon向け**全部入る**
	今回は、CPU向けVulkanDriverの`llvmpipe`を利用しようとしていた
